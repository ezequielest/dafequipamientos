...
if(!class_exists('Cpotheme_Widget_TwitterStream')){
	class Cpotheme_Widget_TwitterStream extends WP_Widget{
		...
		function widget($args, $instance){
			...
			$oauth_path = get_template_directory().'/core/classes/twitteroauth/twitteroauth.php';
			if(defined('WP_CPODEV'))
				$oauth_path = get_template_directory().'/../cpoframework/core/classes/twitteroauth/twitteroauth.php';
			require_once $oauth_path;
			...
			if(is_array($tweets) && count($tweets) > 0){
				$output .= '<ul class="widget-content" id="'.$widget_id.'">';
						...
						if(isset($instance['link_mentions']) && $instance['link_mentions'] == 1){
							...
							if(is_array($mentions)){
								foreach($mentions as $mention_key => $mention_value){
									$contents = str_replace('@'.$mention_value, '<a href="'.esc_url('http://twitter.com/'.$mention_value).'" title="'.sprintf(esc_attr__('@%s on Twitter', 'cpocore'), $mention_value).'">'.'@'.$mention_value.'</a>', $contents);
								}
							}
						}
					}
		...
		function form($instance){
			...
			<p>
				<label for="<?php echo $this->get_field_id('username'); ?>"><?php _e('Twitter Username', 'cpocore'); ?></label><br/>
			...
			<p>
				<label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of Tweets', 'cpocore'); ?></label><br/>
				<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" />
			</p>
			<p><b><?php _e('API Settings', 'cpocore'); ?></b></p>
			<p>
				<label for="<?php echo $this->get_field_id('consumer_key'); ?>"><?php _e('Consumer Key', 'cpocore'); ?></label><br/>
			...
			<p>
				<label for="<?php echo $this->get_field_id('consumer_secret'); ?>"><?php _e('Consumer Secret', 'cpocore'); ?></label><br/>
			...
			<p>
				<label for="<?php echo $this->get_field_id('access_key'); ?>"><?php _e('Access Key', 'cpocore'); ?></label><br/>
			...
			<p>
				<label for="<?php echo $this->get_field_id('access_secret'); ?>"><?php _e('Access Secret', 'cpocore'); ?></label><br/>
