...
if(!class_exists('Cpotheme_Widget_Portfolio')){
	class Cpotheme_Widget_Portfolio extends WP_Widget{
		
		function Cpotheme_Widget_Portfolio(){
			$widget_ops = array('classname' => 'cpotheme_portfolio_items', 'description' => __('Displays elements from the portfolio.', 'cpocore'));
			$this->WP_Widget('cpotheme-portfolio-items', __('CPO - Portfolio Items', 'cpocore'), $widget_ops);
			$this->alt_option_name = 'cpotheme_portfolio_items';
		...
		function widget($args, $instance){
			$cache = wp_cache_get('cpotheme_portfolio_items', 'widget');
			...
			wp_cache_add('cpotheme_portfolio_items', $cache, 'widget');
		}
		...
		function update($new_instance, $old_instance){
			...
			if(isset($alloptions['cpotheme_portfolio_items']))
			delete_option('cpotheme_portfolio_items');
			return $instance;
		}

		function flush_widget_cache(){
			wp_cache_delete('cpotheme_portfolio_items', 'widget');
		}
	...
	register_widget('Cpotheme_Widget_Portfolio'); 
}
